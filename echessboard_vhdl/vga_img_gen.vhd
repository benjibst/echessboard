...
entity VGAImageGenerator is
  port (
    ig_pxl_clk  : in  STD_LOGIC;                     --pixel clock
...
architecture RTL of VGAImageGenerator is
  ...
  signal fb_addr : unsigned(14 downto 0); --framebuffer address
begin
  process (ig_pxl_clk, ig_reset)
    ...
    if (ig_reset = '0') then
      ...
      if (rising_edge(ig_pxl_clk)) then
        ...
        case pixel_in_word is
          when x"0" => fb_pixel_color := ig_fb_data(31 downto 30); --first pixel in word (MSB)
          when x"1" => fb_pixel_color := ig_fb_data(29 downto 28);
          when x"2" => fb_pixel_color := ig_fb_data(27 downto 26);
          when x"3" => fb_pixel_color := ig_fb_data(25 downto 24);
          when x"4" => fb_pixel_color := ig_fb_data(23 downto 22);
          when x"5" => fb_pixel_color := ig_fb_data(21 downto 20);
          when x"6" => fb_pixel_color := ig_fb_data(19 downto 18);
          when x"7" => fb_pixel_color := ig_fb_data(17 downto 16);
          when x"8" => fb_pixel_color := ig_fb_data(15 downto 14);
          when x"9" => fb_pixel_color := ig_fb_data(13 downto 12);
          when x"A" => fb_pixel_color := ig_fb_data(11 downto 10);
          when x"B" => fb_pixel_color := ig_fb_data(9 downto 8);
          when x"C" => fb_pixel_color := ig_fb_data(7 downto 6);
          when x"D" => fb_pixel_color := ig_fb_data(5 downto 4);
          when x"E" => fb_pixel_color := ig_fb_data(3 downto 2);
          when others => fb_pixel_color := ig_fb_data(1 downto 0); --last pixel in word (LSB)
        end case;
        if (ig_disp_ena = '1') then --display time
          ig_red <= color_lut(to_integer(unsigned(fb_pixel_color)))(11 downto 8); --extract red component from color lookup table
          ig_green <= color_lut(to_integer(unsigned(fb_pixel_color)))(7 downto 4); --extract green component from color lookup table
          ig_blue <= color_lut(to_integer(unsigned(fb_pixel_color)))(3 downto 0); --extract blue component from color lookup table
        else --blanking time
          ig_red <= (others => '0');
          ig_green <= (others => '0');
          ig_blue <= (others => '0');
